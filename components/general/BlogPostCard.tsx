...
export function BlogPostCard({data}:IappProps){
    const isValid = data.imageUrl?.startsWith("http");
    return(
        <div className="group relative overflow-hidden rounded-lg border-gray-200 bg-white shadow-md transition-all hover:shadow-lg h-[420px] flex flex-col">
            ...
            <div className="relative h-48 w-full overflow-hidden">
                <Image  src={isValid ? data.imageUrl : "/fallback.jpg"} alt="Blog Image" fill className="object-cover transition-transform duration-300 group-hover:scale-105"/>
            </div>
            <div className="p-4 flex flex-col flex-grow">
                <h3 className="mb-2 text-lg font-semibold text-gray-900">{data.title}</h3>
                <p className="mb-4 text=lg texr-gray-600 line-clamp-2 min-h-[42px]">{data.content}</p>
                <div className=" mt-auto flex items-center justify-between pt-4">
                    ...
                    <time className="text-xs text-gray-500">{data.createdAt.toDateString()}</time>
                </div>
